...
class Cart {
    ...
    add(prod) {
        const existing = this.items.find((i) => i.product.nome === prod.nome);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelector(".produto-container img").src = produto.img;
    document.querySelector(".titulo-preco h2").textContent = produto.nome;
    document.querySelector(".titulo-preco span").textContent = "R$ " + produto.preco.toFixed(2).replace(".", ",");

    // Criar produto e associar botão de adicionar ao carrinho
    const produtoObj = new Product(Date.now(), produto.nome, produto.preco, produto.img);
    ...
    if (btnAdd) {
        btnAdd.addEventListener("click", (e) => {
            e.preventDefault();
            cart.add(produtoObj);
            cartSidebar.classList.add("active"); // abre o carrinho lateral
        });
    }
});
